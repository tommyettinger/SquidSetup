...
public class BuildScriptHelper {
	...
	public static void addBuildScript(List<DependencyBank.ProjectType> projects, BufferedWriter wr) throws IOException {
		...
		if (projects.contains(DependencyBank.ProjectType.HTML)) {
		...
		if (projects.contains(DependencyBank.ProjectType.ANDROID)) {
			write(wr, "classpath '" + DependencyBank.androidPluginImport + "'");
		}
		if (projects.contains(DependencyBank.ProjectType.IOS)) {
	...
	public static void addAllProjects(BufferedWriter wr) throws IOException {
		...
		write(wr, "regexodusVersion = '" + DependencyBank.regexodusVersion + "'");
		write(wr, "}");
		space(wr);
		write(wr, "repositories {");
		write(wr, DependencyBank.mavenCentral);
	...
	private static void addDependencies(DependencyBank.ProjectType project, List<Dependency> dependencyList, BufferedWriter wr) throws IOException {
		write(wr, "dependencies {");
		...
		*/
		for (Dependency dep : dependencyList) {
			...
			for (String moduleDependency : dep.getDependencies(project)) {
				...
				if (project.equals(DependencyBank.ProjectType.ANDROID) && moduleDependency.contains("native")) {
	...
	private static void addConfigurations(DependencyBank.ProjectType project, BufferedWriter wr) throws IOException {
