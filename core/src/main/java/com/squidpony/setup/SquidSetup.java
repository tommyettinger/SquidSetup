...
import com.squidpony.setup.DependencyBank.ProjectDependency;
import com.squidpony.setup.DependencyBank.ProjectType;
import com.squidpony.setup.Executor.CharCallback;
...
public class SquidSetup {
	...
	private byte[] readResource (String resource, String path) {
		...
		try {
			...
			in = SquidSetup.class.getClassLoader().getResourceAsStream(path + resource);
			if (in == null) throw new RuntimeException("Couldn't read resource '" + path + resource + "'");
			int read = 0;
			...
			throw new RuntimeException("Couldn't read resource '" + path + resource + "'", e);
		} finally {
			if (in != null) try {
				in.close();
			} catch (IOException e) {
	...
	private static void printHelp () {
		System.out
			.println("Usage: SquidSetup --dir <dir-name> --name <app-name> --package <package> --mainClass <mainClass> --sdkLocation <SDKLocation>");
	...
	public static void main (String[] args) throws IOException {
		...
		if (!params.containsKey("dir") ||
			...
			builder.build();
			new SquidSetup().build(builder, params.get("dir"), params.get("name"), params.get("package"), params.get("mainClass"),
