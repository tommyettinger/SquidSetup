...
public class SquidSetupUI extends JFrame {

	//DependencyBank dependencyBank;
	ProjectBuilder builder;
	List<DependencyBank.ProjectType> modules = new ArrayList<DependencyBank.ProjectType>();
	...
	public SquidSetupUI() {
		setTitle("SquidLib Project Setup Tool");
		...
		modules.add(DependencyBank.ProjectType.CORE);
		dependencies.add(builder.bank.getDependency(DependencyBank.ProjectDependency.SQUIDLIB));
		dependencies.add(builder.bank.getDependency(DependencyBank.ProjectDependency.SQUIDLIB_UTIL));
	}
	...
	void generate () {
		...
		if (sdkLocation.length() == 0 && modules.contains(DependencyBank.ProjectType.ANDROID)) {
			JOptionPane.showMessageDialog(this, "Please enter your Android SDK's path");
			return;
		}
		if (!SquidSetup.isSdkLocationValid(sdkLocation) && modules.contains(DependencyBank.ProjectType.ANDROID)) {
			JOptionPane
		...
		if (modules.contains(DependencyBank.ProjectType.ANDROID)) {
			if (!SquidSetup.isSdkUpToDate(sdkLocation)) {
		...
		if (!SquidSetup.isEmptyDirectory(destination)) {
		...
		new Thread() {
			public void run () {
				...
				new SquidSetup().build(builder, destination, name, pack, clazz, sdkLocation, new Executor.CharCallback() {
					@Override
	...
	class UI extends JPanel {
...

		{
			...
			try {
				BufferedImage exitimg = ImageIO.read(SquidSetupUI.class.getResourceAsStream("/exitup.png"));
				BufferedImage minimg = ImageIO.read(SquidSetupUI.class.getResourceAsStream("/minimizeup.png"));
				BufferedImage exitimgdown = ImageIO.read(SquidSetupUI.class.getResourceAsStream("/exitdown.png"));
				BufferedImage minimgdown = ImageIO.read(SquidSetupUI.class.getResourceAsStream("/minimizedown.png"));
				BufferedImage exithover = ImageIO.read(SquidSetupUI.class.getResourceAsStream("/exithover.png"));
				BufferedImage minimghover = ImageIO.read(SquidSetupUI.class.getResourceAsStream("/minimizehover.png"));

				BufferedImage img = ImageIO.read(SquidSetupUI.class.getResourceAsStream("/logo.png"));
	...
	class Form extends JPanel {
		...
		JTextField nameText = new JTextField("my-squid-game");
		JLabel packageLabel = new JLabel("Package:");
		JTextField packageText = new JTextField("com.mysquid.game");
		JLabel gameClassLabel = new JLabel("Game class:");
		JTextField gameClassText = new JTextField("MySquidGame");
		...
		private void uiStyle() {
			...
			projectsLabel.setForeground(new Color(194, 157, 235));
			extensionsLabel.setForeground(new Color(194, 157, 235));
		...
		private void uiLayout () {
			...
			for (int row = 0; row <= (DependencyBank.ProjectDependency.values().length / 5); row++) {
				JPanel extensionPanel = new JPanel(new GridLayout(1, 5));
				while (depCounter < DependencyBank.ProjectDependency.values().length) {
					if (DependencyBank.ProjectDependency.values()[depCounter] != null) {
						final DependencyBank.ProjectDependency projDep = DependencyBank.ProjectDependency.values()[depCounter];
						if (projDep.equals(DependencyBank.ProjectDependency.SQUIDLIB_UTIL)) {
							depCounter++;
							continue;
						}
						...
						if (projDep.equals(DependencyBank.ProjectDependency.SQUIDLIB)) {
		...
		File getDirectory () {
			if (System.getProperty("os.name").contains("Mac")) {
				...
				FileDialog dialog = new FileDialog(SquidSetupUI.this, "Choose destination", FileDialog.LOAD);
	...
	public static class SetupCheckBox extends JCheckBox {
		...
		private static BufferedImage getCheckboxImage (String imageName) throws IOException {
			return ImageIO.read(SquidSetupUI.class.getResourceAsStream("/" + imageName + ".png"));
		}
